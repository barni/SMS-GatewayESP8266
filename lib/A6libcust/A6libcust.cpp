...
A6libcust::A6libcust(int transmitPin, int receivePin) {
...
byte A6libcust::blockUntilReady(long baudRate) {
...
byte A6libcust::begin(long baudRate) {
    A6conn->flush();
...
void A6libcust::powerCycle(int pin) {
...
void A6libcust::powerOff(int pin) {
    pinMode(pin, OUTPUT);
...
void A6libcust::powerOn(int pin) {
    pinMode(pin, OUTPUT);
...
// Dial a number.
void A6libcust::dial(String number) {
    char buffer[50];
...
byte A6libcust::pin(String pin) {
    ...
    sprintf(buffer, "AT+CPIN=\"%s\"", pin.c_str());

    A6command("AT+CSCA?", "OK", "yy", A6_CMD_TIMEOUT, 2, NULL);
    A6command(buffer, "OK", "yy", A6_CMD_TIMEOUT, 2, NULL);
    sdelay(100);
...
bool A6libcust::isConnected(){
  String response;
  A6command("AT+CREG?", "OK", "yy", A6_CMD_TIMEOUT, 4, &response);
  response.trim();
  logln(response);
  ...
  //+CREG: 1,1 or +CREG: 1,5 1:HOME, 5:ROAMING
  return response.substring(index+1,index+2).endsWith("1") || response.substring(index+1,index+2).endsWith("5");
}
// Send an SMS.
byte A6libcust::sendSMS(String number, String text) {
    char ctrlZ[2] = { 0x1a, 0x00 };
    char buffer[100];
    ...
    A6command(buffer, ">", "yy", A6_CMD_TIMEOUT, 4, NULL);
    sdelay(100);
    A6conn->println(text.c_str());
    A6conn->println(ctrlZ);
    A6conn->println();
...
int A6libcust::getUnreadSMSLocs(int* buf, int maxItems) {
...
SMSmessage A6libcust::readSMS(int index) {
    String response = "";
    char buffer[30];
...
byte A6libcust::deleteSMS(int index) {
    char buffer[20];
...
// Set the SMS charset.
byte A6libcust::setSMScharset(String charset) {
    char buffer[30];
...
// 8 inclusive.
void A6libcust::setVol(byte level) {
    char buffer[30];
...
void A6libcust::enableSpeaker(byte enable) {
    char buffer[30];
...
char A6libcust::setRate(long baudRate) {
    int rate = 0;
...
// Issue a command.
byte A6libcust::A6command(const char *command, const char *resp1, const char *resp2, int timeout, int repetitions, String *response) {
...
// Wait for responses.
byte A6libcust::A6waitFor(const char *resp1, const char *resp2, int timeout, String *response) {
